...
public class TsFileInputFormat<T> extends FileInputFormat<T> implements ResultTypeQueryable<T> {
    ...
    public void open(FileInputSplit split) throws IOException {
                ...
                in =
                        new HDFSInput(
                                new org.apache.hadoop.fs.Path(
                                        new URI(currentSplit.getPath().getPath())),
                                hadoopConf);
            } else {
                // Local File System
        ...
        try (TsFileSequenceReader reader = new TsFileSequenceReader(in)) {
            ...
            queryDataSet =
                    readTsFile.query(
                            // The query method call will change the content of the param query
                            // expression,
                            // the original query expression should not be passed to the query
                            // method as it may
                            // be used several times.
                            QueryExpression.create(
                                    expression.getSelectedSeries(), expression.getExpression()),
