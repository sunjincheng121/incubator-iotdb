...
public class AsyncClientPool {
    ...
    public AsyncClient getClient(Node node) throws IOException {
        ...
        if (blockOnError
                && nodeErrorClientCountMap.getOrDefault(clusterNode, 0) > MAX_ERROR_COUNT) {
            throw new IOException(
                    String.format("connect node failed, maybe the node is down, %s", node));
        }
     ...
     * client is returned beyond the timeout, a new client will be returned. WARNING: the caller
     * must synchronize on the pool.
     *
     * @param clientStack
     * @param node
     * @param nodeClientNum
    ...
    void onError(Node node) {
        ...
        synchronized (this) {
            if (nodeErrorClientCountMap.containsKey(clusterNode)) {
                nodeErrorClientCountMap.put(
                        clusterNode, nodeErrorClientCountMap.get(clusterNode) + 1);
            } else {
