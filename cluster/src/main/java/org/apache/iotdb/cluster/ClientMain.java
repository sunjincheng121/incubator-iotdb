...
public class ClientMain {
    ...
    static {
        ...
        options.addOption(
                new Option(
                        PARAM_QUERY_PORTS,
                        true,
                        "Ports to query (ip is currently " + "localhost)"));
            ...
            for (int i = 0; i < MEASUREMENTS.length; i++) {
                ...
                schemas.add(
                        new MeasurementSchema(
                                device + IoTDBConstant.PATH_SEPARATOR + measurement,
                                DATA_TYPES[i]));
            }
        }
    }
    ...
    private static void executeQuery(Client client, long sessionId, String query, long statementId)
        ...
        TSExecuteStatementResp resp =
                client.executeQueryStatement(
                        new TSExecuteStatementReq(sessionId, query, statementId)
                                .setFetchSize(1000));
    ...
    private static void testDeleteStorageGroup(Client client, long sessionId)
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                    client.deleteStorageGroups(sessionId, Arrays.asList(STORAGE_GROUPS))
                            .toString());
        }
    ...
    private static void testBatch(String ip, int port) throws ClassNotFoundException, SQLException {
        ...
        try (Connection connection =
                        DriverManager.getConnection(
                                Config.IOTDB_URL_PREFIX + String.format("%s:%d/", ip, port),
                                "root",
                                "root");
