...
public class FilePartitionedSnapshotLogManager extends PartitionedSnapshotLogManager<FileSnapshot> {
    ...
    private void collectTsFiles() throws IOException {
        ...
        for (Entry<PartialPath, Map<Long, List<TsFileResource>>> entry :
            ...
            for (Entry<Long, List<TsFileResource>> storageGroupFiles :
                    storageGroupsFiles.entrySet()) {
                Long partitionNum = storageGroupFiles.getKey();
                List<TsFileResource> resourceList = storageGroupFiles.getValue();
                if (!collectTsFiles(
                        partitionNum, resourceList, storageGroupName, createdHardlinks)) {
                    // some file is deleted during the collecting, clean created hardlinks and
                    // restart
                    // from the beginning
    ...
    private boolean collectTsFiles(
        ...
        for (TsFileResource tsFileResource : resourceList) {
            ...
            logger.debug(
                    "{}: File {} is put into snapshot #{}", getName(), tsFileResource, slotNum);
            snapshot.addFile(
                    hardlink, thisNode, isPlanIndexRangeUnique(tsFileResource, resourceList));
        }
