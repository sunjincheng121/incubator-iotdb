    ...
    private static void doRemoveNode(String[] args) throws IOException {
        ...
        for (String url : config.getSeedNodeUrls()) {
            ...
            AsyncMetaClient client =
                    new AsyncMetaClient(factory, new TAsyncClientManager(), node, null);
            Long response = null;
            try {
    ...
    private static void preStartCustomize() {
        ...
        SlotPartitionTable.setSlotStrategy(
                new SlotStrategy() {
                    ...
                    @Override
                    public int calculateSlotByTime(
                            String storageGroupName, long timestamp, int maxSlotNum) {
                        int sgSerialNum = extractSerialNumInSGName(storageGroupName) % k;
                        if (sgSerialNum >= 0) {
                            return maxSlotNum / k * sgSerialNum;
                        } else {
                            return defaultStrategy.calculateSlotByTime(
                                    storageGroupName, timestamp, maxSlotNum);
                        }
                    }
