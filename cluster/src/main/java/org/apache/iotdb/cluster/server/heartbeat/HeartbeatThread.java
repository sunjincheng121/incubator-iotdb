...
public class HeartbeatThread implements Runnable {
    ...
    public void run() {
        ...
        long electionWait =
                ClusterConstant.ELECTION_LEAST_TIME_OUT_MS
                ...
                switch (localMember.getCharacter()) {
                    ...
                    case FOLLOWER:
                        ...
                        if (heartBeatInterval >= RaftServer.getConnectionTimeoutInMS()) {
                            // the leader is considered dead, an election will be started in the
                            // next loop
                            logger.info(
                                    "{}: The leader {} timed out",
                                    memberName,
                    ...
                    .submit(
                            () -> {
                                try {
                                    ...
                                    logger.warn(
                                            "{}: Cannot send heart beat to node {}",
                                            memberName,
                                            node,
                                            e);
                                } finally {
