...
public class DataClusterServer extends RaftServer
    ...
    public DataClusterServer(
            Node thisNode,
            DataGroupMember.Factory dataMemberFactory,
     ...
     * longer stays in that group, also remove and stop it. If the new group contains this node,
     * also create and add a new DataGroupMember for it.
     *
     * @param node
     * @param result
     */
    public void addNode(Node node, NodeAdditionResult result) {
        ...
        synchronized (headerGroupMap) {
            while (entryIterator.hasNext()) {
                ...
                if (shouldLeave) {
                    logger.info(
                            "This node does not belong to {} any more",
    ...
    /**
     * Try removing a node from the groups of each DataGroupMember. If the node is the header of
     * some group, set the member to read only so that it can still provide data for other nodes
     * that has not yet pulled its data. If the node is the local node, remove all members whose
     * group is not headed by this node. Otherwise, just change the node list of the member and pull
     * new data. And create a new DataGroupMember if this node should join a new group because of
     * this removal.
     *
     * @param node
        ...
        synchronized (headerGroupMap) {
            while (entryIterator.hasNext()) {
                ...
                if (dataGroupMember.getHeader().equals(node)) {
                    // the group is removed as the node is removed, so new writes should be rejected
                    // as
                    // they belong to the new holder, but the member is kept alive for other nodes
                    // to pull
                    // snapshots
                    entryIterator.remove();
                    removeMember(entry.getKey(), entry.getValue());
    ...
    @Override
    public ByteBuffer getAllMeasurementSchema(Node header, ByteBuffer planBinary)
            throws TException {
