...
public class CatchUpTask implements Runnable {
    ...
    /**
     * @return true if a matched index is found so that we can use logs only to catch up, or false
     *     if the catch up must be done with a snapshot.
     * @throws TException
     ...
     * @return true if the previous log at logs[index] matches a log in the remote node, false if
     *     the corresponding log cannot be found
     * @throws LeaderUnknownException
     * @throws TException
     * @throws InterruptedException
     */
    private boolean checkMatchIndex(int index)
        ...
        if (prevLogTerm == -1) {
            // prev log cannot be found, we cannot know whether is matches if it is not the first
            // log
    ...
    private long getPrevLogTerm(int index) {
        ...
        if (index > 0) {
            ...
            try {
                ...
                logger.info(
                        "Log [{}] is compacted during catchup", logs.get(0).getCurrLogIndex() - 1);
            }
        }
        return prevLogTerm;
    }
    ...
    private void removeSnapshotLogs() {
        Log logToSearch =
                new EmptyContentLog(snapshot.getLastLogIndex(), snapshot.getLastLogTerm());
        int pos =
                Collections.binarySearch(
                        logs, logToSearch, Comparator.comparingLong(Log::getCurrLogIndex));
        ...
        try {
            ...
            if (!findMatchedIndex) {
                ...
                SnapshotCatchUpTask task =
                        new SnapshotCatchUpTask(logs, snapshot, node, raftMember);
