...
public class CommittedEntryManager {
    ...
    /**
     * Overwrite the contents of this object with those of the given snapshot. Note that this
     * function is only used if you want to override all the contents, otherwise please use
     ...
     *
     * @param index request entry index
     * @return null if index > entries[entries.size()-1].index, throw EntryCompactedException if
     *     index < dummyIndex, or return the entry's log for given index
     * @throws EntryCompactedException
     */
    ...
    void compactEntries(long compactIndex) throws EntryUnavailableException {
        ...
        if (compactIndex > getLastIndex()) {
            logger.info(
                    "compact ({}) is out of bound lastIndex ({})", compactIndex, getLastIndex());
