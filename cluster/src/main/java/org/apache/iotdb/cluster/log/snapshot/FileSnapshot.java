...
public class FileSnapshot extends Snapshot implements TimeseriesSchemaSnapshot {
    ...
    public static class Installer implements SnapshotInstaller<FileSnapshot> {
        ...
        /**
         * When a DataGroupMember pulls data from another node, the data files will be firstly
         * stored in the "REMOTE_FILE_TEMP_DIR", and then load file functionality of IoTDB will be
         * used to load the files into the IoTDB instance.
         */
        ...
        private void installFileSnapshotFiles(FileSnapshot snapshot, int slot)
            ...
            for (int i = 0, remoteTsFileResourcesSize = remoteTsFileResources.size();
                ...
                logger.info(
                        "Pulling {}/{} files, current: {}",
                        i + 1,
                        remoteTsFileResources.size(),
                        resource);
                try {
         ...
         * is controlled by the data group leader, the files with the same version should contain
         * identical data if without merge. Even with merge, the files that the merged file is from
         * are recorded so we can still find out if the data of a file is already replicated in this
         * member.
         *
         * @param resource
         * @return
        ...
        private boolean isFileAlreadyPulled(RemoteTsFileResource resource)
            ...
            return StorageEngine.getInstance()
                    .isFileAlreadyExist(
                            resource, new PartialPath(storageGroupName), partitionNumber);
        }
        ...
        private void removeRemoteHardLink(RemoteTsFileResource resource) {
            ...
            if (ClusterDescriptor.getInstance().getConfig().isUseAsyncServer()) {
                AsyncDataClient client =
                        (AsyncDataClient) dataGroupMember.getAsyncClient(sourceNode);
                if (client != null) {
                    try {
        ...
        /**
         * Load a remote file from the header of the data group that the file is in. As different
         * IoTDB instances will name the file with the same version differently, we can only pull
         * the file from the header currently.
         *
         * @param resource
         */
         ...
         * resource and remove the files that is a subset of the new file. Also change the
         * modification file if the new file is with one.
         *
         * @param resource
         */
        private void loadRemoteResource(RemoteTsFileResource resource) throws IllegalPathException {
            ...
            try {
                ...
                if (resource.isPlanRangeUnique()) {
                    // only when a file has a unique range can we remove other files that over lap
                    // with it,
                ...
                logger.error(
                        "{}: Cannot load remote file {} into storage group", name, resource, e);
                return;
            }
            if (remoteModFile.exists()) {
                // when successfully loaded, the filepath of the resource will be changed to the
                // IoTDB data
                // dir, so we can add a suffix to find the old modification file.
                File localModFile =
                        new File(
                                resource.getTsFile().getAbsoluteFile()
         ...
         * @return the downloaded file or null if the file cannot be downloaded or its MD5 is not
         *     right
         * @throws IOException
         */
        private File pullRemoteFile(RemoteTsFileResource resource, Node node) throws IOException {
            ...
            String tempFileName =
                    pathSegments[segSize - 1].substring(
                            0, pathSegments[segSize - 1].lastIndexOf('.'));
            String tempFilePath =
                    node.getNodeIdentifier()
                            + File.separator
                            + pathSegments[segSize - 3]
                            + File.separator
                            + pathSegments[segSize - 2]
                            + File.separator
                            + tempFileName;
            ...
            if (pullRemoteFile(resource.getTsFile().getAbsolutePath(), node, tempFile)) {
                // TODO-Cluster#353: implement file examination, may be replaced with other
                // algorithm
        ...
        /**
         * Download the file "remotePath" from "node" and store it to "dest" using up to 64KB
         * chunks. If the network is bad, this method will retry upto 5 times before returning a
         * failure.
         *
        ...
        private boolean pullRemoteFile(String remotePath, Node node, File dest) throws IOException {
            ...
            for (int i = 0; i < pullFileRetry; i++) {
                try (BufferedOutputStream bufferedOutputStream =
                    ...
                    logger.warn(
                            "{}: Pulling file {} from {} interrupted", name, remotePath, node, e);
                    return false;
                }
                ...
                try {
                    ...
                    logger.warn(
                            "Cannot delete file when pulling {} from {} failed", remotePath, node);
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
                    logger.warn(
                            "{}: Pulling file {} from {} interrupted", name, remotePath, node, ex);
                    return false;
                }
                // next try
            }
        ...
        private void downloadFileAsync(Node node, String remotePath, OutputStream dest)
            ...
            while (true) {
                ...
                ByteBuffer buffer =
                        SyncClientAdaptor.readFile(client, remotePath, offset, fetchSize);
    ...
    public static class Factory implements SnapshotFactory<FileSnapshot> {
        ...
        public FileSnapshot copy(FileSnapshot origin) {
            ...
            fileSnapshot.dataFiles =
                    origin.dataFiles == null ? null : new ArrayList<>(origin.dataFiles);
            fileSnapshot.timeseriesSchemas =
                    origin.timeseriesSchemas == null
                            ? null
