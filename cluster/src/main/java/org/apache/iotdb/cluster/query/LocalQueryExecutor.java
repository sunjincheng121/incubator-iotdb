...
public class LocalQueryExecutor {
    ...
    public ByteBuffer fetchSingleSeriesByTimestamp(long readerId, long time)
        ...
        IReaderByTimestamp reader =
                dataGroupMember.getQueryManager().getReaderByTimestamp(readerId);
        if (reader == null) {
    ...
    /**
     * Send the timeseries schemas of some prefix paths to the requester. The schemas will be sent
     * in the form of a list of MeasurementSchema, but notice the measurements in them are the full
     * paths.
     *
     * @param request
     */
    ...
    public List<ByteBuffer> getAggrResult(GetAggrResultRequest request)
        ...
        results =
                getAggrResult(
                        ...
                        ascending);
        logger.trace(
                "{}: aggregation results {}, queryId: {}", name, results, request.getQueryId());
        ...
        for (String seriesPath : timeseriesList) {
            try {
                List<PartialPath> path =
                        getCMManager().getAllTimeseriesPath(new PartialPath(seriesPath));
                if (path.size() != 1) {
                    throw new MetadataException(
                            String.format(
                                    "Timeseries number of the name [%s] is not 1.", seriesPath));
                }
    ...
    public long getGroupByExecutor(GroupByRequest request)
        ...
        RemoteQueryContext queryContext =
                queryManager.getQueryContext(
                        request.getRequestor(), queryId, DEFAULT_FETCH_SIZE, -1);
        LocalGroupByExecutor executor =
                getGroupByExecutor(
                        path,
                        deviceMeasurements,
    ...
    public List<ByteBuffer> getGroupByResult(long executorId, long startTime, long endTime)
        ...
        logger.debug(
                "{}: Send results of group by executor {}, size:{}",
                name,
                executor,
                resultBuffers.size());
        return resultBuffers;
    ...
    public ByteBuffer previousFill(PreviousFillRequest request)
        ...
        TimeValuePair timeValuePair =
                localPreviousFill(
                        path, dataType, queryTime, beforeRange, deviceMeasurements, queryContext);
    ...
    public ByteBuffer last(LastQueryRequest request)
            throws CheckConsistencyException, QueryProcessException, IOException,
                    StorageEngineException, IllegalPathException {
        dataGroupMember.syncLeaderWithConsistencyCheck(false);
