...
public class ClusterConfig {
    ...
    /**
     * Size of log buffer. If raft log persistence is enabled and the size of a insert plan is
     * smaller than this parameter, then the insert plan will be rejected by WAL.
     */
     ...
     * means the server will first try to synchronize with the leader, but if failed(timeout), it
     * will give up and just use current data it has cached before; Weak consistency do not
     * synchronize with the leader and simply use the local data
     */
    ...
    /**
     * The maximum value of the raft log index stored in the memory per raft group, These indexes
     * are used to index the location of the log on the disk
     */
