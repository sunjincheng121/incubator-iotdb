...
public class MetaSimpleSnapshot extends Snapshot {
        ...
        for (int i = 0; i < storageGroupTTLMapSize; i++) {
            try {
                storageGroupTTLMap.put(
                        new PartialPath(SerializeUtils.deserializeString(buffer)),
                        buffer.getLong());
    ...
    public static class Installer implements SnapshotInstaller<MetaSimpleSnapshot> {
        ...
        /**
         * Install a meta snapshot to IoTDB. The snapshot contains: all storage groups, partition
         * table, authentication info, and last log term/index in the snapshot.
         */
        private void installSnapshot(MetaSimpleSnapshot snapshot) {
            synchronized (metaGroupMember.getSnapshotApplyLock()) {
                ...
                for (Map.Entry<PartialPath, Long> entry :
                        snapshot.getStorageGroupTTLMap().entrySet()) {
                ...
                try {
                    ...
                    logger.error(
                            "{}: Cannot get authorizer instance, error is: ",
                            metaGroupMember.getName(),
                            e);
                }
