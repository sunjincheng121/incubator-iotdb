...
public class ElectionHandler implements AsyncMethodCallback<Long> {
    ...
    public void onComplete(Long resp) {
        ...
        synchronized (raftMember.getTerm()) {
            if (terminated.get()) {
                // a voter has rejected this election, which means the term or the log id falls
                // behind
            ...
            if (voterResp == RESPONSE_AGREE) {
                ...
                if (voterResp < currTerm) {
                    // the rejection from a node with a smaller term means the log of this node
                    // falls behind
                    logger.info(
                            "{}: Election {} rejected: code {}", memberName, currTerm, voterResp);
                    onFail();
                } else {
                    // the election is rejected by a node with a bigger term, update current term to
                    // it
                    logger.info(
