...
public class UnCommittedEntryManager {
     ...
     * entries[entries.size()-1].index + 1. Note that the caller should ensure not to truncate
     * entries which have been committed.
     *
    ...
    void truncateAndAppend(List<Log> appendingEntries) {
        ...
        if (maybeTerm(firstAppendingEntry.getCurrLogIndex()) == firstAppendingEntry.getCurrLogTerm()
                && maybeTerm(lastAppendingEntry.getCurrLogIndex())
                        == lastAppendingEntry.getCurrLogTerm()) {
            // skip existing entry
            return;
        }
        ...
        if (len < 0) {
            ...
            // entries
            logger.error(
                    "The logs which first index is {} are going to truncate committed logs", after);
    ...
    void truncateAndAppend(Log appendingEntry) {
        ...
        if (len < 0) {
            ...
            // then append
            logger.info(
                    "truncate the entries after index {}, append a new entry {}",
                    after,
                    appendingEntry);
            int truncateIndex = (int) (after - offset);
            if (truncateIndex < entries.size()) {
    ...
    public List<Log> getEntries(long low, long high) {
        if (low > high) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "invalid unCommittedEntryManager getEntries: parameter: low({}) > high({})",
                        low,
                        high);
            }
