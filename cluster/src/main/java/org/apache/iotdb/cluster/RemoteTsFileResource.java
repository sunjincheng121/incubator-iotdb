...
public class RemoteTsFileResource extends TsFileResource {
    ...
    /**
     * Whether the plan range ([minPlanIndex, maxPlanIndex]) overlaps with another TsFile in the
     * same time partition. If not (unique = true), we shall have confidence that the file has all
     * data whose plan indexes are within [minPlanIndex, maxPlanIndex], so we can remove other local
    ...
    public void serialize(DataOutputStream dataOutputStream) {
        ...
        try {
            // the path here is only for the remote node to get a download link, so it does not
            // matter
            // if it is absolute
