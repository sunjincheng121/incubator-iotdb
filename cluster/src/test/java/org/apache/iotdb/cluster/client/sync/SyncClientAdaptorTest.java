...
public class SyncClientAdaptorTest {
                ...
                new AsyncDataClient(null, null, null) {
                    ...
                    public void getUnregisteredTimeseries(
                        ...
                        resultHandler.onComplete(
                                timeseriesList.subList(0, timeseriesList.size() / 2));
                    }
    ...
    public void testMetaClient() throws TException, InterruptedException, IOException {
        ...
        assertTrue(
                SyncClientAdaptor.matchTerm(
                        metaClient, TestUtils.getNode(0), 1, 1, TestUtils.getNode(0)));
    ...
    public void testDataClient()
        ...
        assertEquals(
                1L,
                (long)
                        SyncClientAdaptor.querySingleSeries(
                                dataClient, new SingleSeriesQueryRequest(), 0));
        assertEquals(
                Arrays.asList("1", "2", "3"),
        ...
        assertEquals(
                aggregateResults,
                SyncClientAdaptor.getAggrResult(dataClient, new GetAggrResultRequest()));
        assertEquals(
                paths.subList(0, paths.size() / 2),
                SyncClientAdaptor.getUnregisteredMeasurements(
                        dataClient, TestUtils.getNode(0), paths));
        assertEquals(
                paths,
        ...
        assertEquals(
                1L, (long) SyncClientAdaptor.getGroupByExecutor(dataClient, new GroupByRequest()));
        assertEquals(
                fillResult, SyncClientAdaptor.previousFill(dataClient, new PreviousFillRequest()));
