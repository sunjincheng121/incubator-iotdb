...
public class Session {
    ...
    public void createTimeseries(
        ...
        TSCreateTimeseriesReq request =
                genTSCreateTimeseriesReq(
                        path, dataType, encoding, compressor, null, null, null, null);
    ...
    /**
     * query eg. select * from paths where time >= startTime and time < endTime time interval
     * include startTime and exclude endTime
     *
     * @param paths
    ...
    public void insertRecord(
            ...
            throws IoTDBConnectionException, StatementExecutionException {
        TSInsertRecordReq request =
                genTSInsertRecordReq(deviceId, time, measurements, types, values);
        insertRecord(deviceId, request);
    }
     ...
     * executeBatch, we pack some insert request in batch and send them to server. If you want
    ...
    private void insertStringRecordsWithLeaderCache(
        ...
        for (int i = 0; i < deviceIds.size(); i++) {
            TSInsertStringRecordsReq request =
                    deviceGroup.computeIfAbsent(
                            deviceIds.get(i), k -> new TSInsertStringRecordsReq());
