...
public class IoTDBSessionIteratorIT {
            ...
            while (iterator.next()) {
                String ans =
                        String.format(
                                ...
                                iterator.getFloat(3),
                                iterator.getString("root.sg1.d2.s1"));
    ...
    public void test2() {
        String[] retArray =
                new String[] {"9,root.sg1.d1.s1,1", "9,root.sg1.d1.s2,2.0", "9,root.sg1.d2.s1,4.0"};

        try {
            SessionDataSet sessionDataSet =
                    session.executeQueryStatement("select last * from root.sg1");
            sessionDataSet.setFetchSize(1024);
            DataIterator iterator = sessionDataSet.iterator();
            int count = 0;
            while (iterator.next()) {
                String ans =
                        String.format(
                                "%s,%s,%s",
                                iterator.getLong(1), iterator.getString(2), iterator.getString(3));
                assertEquals(retArray[count], ans);
                count++;
            }
            assertEquals(retArray.length, count);
    ...
    private void prepareData() throws IoTDBConnectionException, StatementExecutionException {
        ...
        session.createTimeseries(
                "root.sg1.d1.s1", TSDataType.INT32, TSEncoding.RLE, CompressionType.SNAPPY);
        session.createTimeseries(
                "root.sg1.d1.s2", TSDataType.FLOAT, TSEncoding.RLE, CompressionType.SNAPPY);
        session.createTimeseries(
                "root.sg1.d2.s1", TSDataType.DOUBLE, TSEncoding.RLE, CompressionType.SNAPPY);
