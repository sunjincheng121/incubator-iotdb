...
public class ChunkWriterImpl implements IChunkWriter {
    ...
    private void checkSdtEncoding() {
        if (measurementSchema.getProps() != null && !isMerging) {
            ...
            if (isSdtEncoding && measurementSchema.getProps().containsKey("compdev")) {
                try {
                    sdtEncoder.setCompDeviation(
                            Double.parseDouble(measurementSchema.getProps().get("compdev")));
                } catch (NumberFormatException e) {
                    logger.error("meet error when formatting SDT compression deviation");
                }
                if (sdtEncoder.getCompDeviation() < 0) {
                    logger.error(
                            "SDT compression deviation cannot be negative. SDT encoding is turned off.");
                    isSdtEncoding = false;
                }
            }
            ...
            if (isSdtEncoding && measurementSchema.getProps().containsKey("compmin")) {
                try {
                    sdtEncoder.setCompMin(
                            Double.parseDouble(measurementSchema.getProps().get("compmin")));
                } catch (NumberFormatException e) {
            ...
            if (isSdtEncoding && measurementSchema.getProps().containsKey("compmax")) {
                try {
                    sdtEncoder.setCompMax(
                            Double.parseDouble(measurementSchema.getProps().get("compmax")));
                } catch (NumberFormatException e) {
    ...
    private void checkPageSizeAndMayOpenANewPage() {
        if (pageWriter.getPointNumber() == maxNumberOfPointsInPage) {
            logger.debug(
                    "current line count reaches the upper bound, write page {}", measurementSchema);
            ...
            if (currentPageSize > pageSizeThreshold) { // memory size exceeds threshold
                ...
                valueCountInOnePageForNextCheck =
                        (int)
                                (((float) pageSizeThreshold / currentPageSize)
                                        * pageWriter.getPointNumber());
            }
        }
    }
    ...
    private void writePageToPageBuffer() {
        try {
            ...
            logger.error(
                    "meet error in pageWriter.writePageHeaderAndDataIntoBuff,ignore this page:", e);
        } finally {
    ...
    public long getCurrentChunkSize() {
        ...
        return ChunkHeader.getSerializedSize(measurementSchema.getMeasurementId())
