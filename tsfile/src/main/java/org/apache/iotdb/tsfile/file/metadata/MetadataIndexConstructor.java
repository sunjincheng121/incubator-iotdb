    ...
    public static MetadataIndexNode constructMetadataIndex(
        ...
        for (Entry<String, List<TimeseriesMetadata>> entry :
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                // when constructing from leaf node, every "degree number of nodes" are related to
                // an entry
                if (i % MAX_DEGREE_OF_INDEX_NODE == 0) {
                    if (currentIndexNode.isFull()) {
                        addCurrentIndexNodeToQueue(
                                currentIndexNode, measurementMetadataIndexQueue, out);
                        currentIndexNode =
                                new MetadataIndexNode(MetadataIndexNodeType.LEAF_MEASUREMENT);
                    }
        ...
        if (deviceMetadataIndexMap.size() <= MAX_DEGREE_OF_INDEX_NODE) {
            MetadataIndexNode metadataIndexNode =
                    new MetadataIndexNode(MetadataIndexNodeType.INTERNAL_MEASUREMENT);
            for (Map.Entry<String, MetadataIndexNode> entry : deviceMetadataIndexMap.entrySet()) {
                metadataIndexNode.addEntry(
                        new MetadataIndexEntry(entry.getKey(), out.getPosition()));
        ...
        MetadataIndexNode deviceMetadataIndexNode =
                generateRootNode(
                        deviceMetadaIndexQueue, out, MetadataIndexNodeType.INTERNAL_DEVICE);
        deviceMetadataIndexNode.setEndOffset(out.getPosition());
        return deviceMetadataIndexNode;
    }

    /**
     * Generate root node, using the nodes in the queue as leaf nodes. The final metadata tree has
     * two levels: measurement leaf nodes will generate to measurement root node; device leaf nodes
     * will generate to device root node
     *
    ...
    private static MetadataIndexNode generateRootNode(
        ...
        while (queueSize != 1) {
            for (int i = 0; i < queueSize; i++) {
                ...
                currentIndexNode.addEntry(
                        new MetadataIndexEntry(
                                metadataIndexNode.peek().getName(), out.getPosition()));
