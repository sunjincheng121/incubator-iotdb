...
public class SDTEncoder {
    ...
    /**
     * the last read time and value if upperDoor >= lowerDoor meaning out of compDeviation range,
     * will store lastReadPair
     */
     ...
     * stored point time distance <= compMin, current point will NOT be stored regardless of
     ...
     * stored point time distance >= compMax, current point will be stored regardless of compression
     * deviation
     */
    ...
    public boolean encodeFloat(long time, float value) {
        ...
        double curUpperSlope =
                (value - lastStoredFloat - compDeviation) / (time - lastStoredTimestamp);
    ...
    public boolean encodeLong(long time, long value) {
        ...
        double curUpperSlope =
                (value - lastStoredLong - compDeviation) / (time - lastStoredTimestamp);
        if (curUpperSlope > upperDoor) {
            upperDoor = curUpperSlope;
        }
    ...
    public boolean encodeInt(long time, int value) {
        ...
        double curUpperSlope =
                (value - lastStoredInt - compDeviation) / (time - lastStoredTimestamp);
        if (curUpperSlope > upperDoor) {
            upperDoor = curUpperSlope;
        }
    ...
    /**
     * if current point to the last stored point's time distance >= compMax, will store current
