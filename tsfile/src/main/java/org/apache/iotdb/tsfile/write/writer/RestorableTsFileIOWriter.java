    ...
    public RestorableTsFileIOWriter(File file) throws IOException {
        ...
        if (file.exists()) {
            try (TsFileSequenceReader reader =
                ...
                truncatedSize =
                        reader.selfCheck(knownSchemas, chunkGroupMetadataList, versionInfo, true);
    ...
    public List<ChunkMetadata> getVisibleMetadataList(
        ...
        if (metadatasForQuery.containsKey(deviceId)
            ...
            for (ChunkMetadata chunkMetaData : metadatasForQuery.get(deviceId).get(measurementId)) {
                // filter: if a device'measurement is defined as float type, and data has been
                // persistent.
                // Then someone deletes the timeseries and recreate it with Int type. We have to
                // ignore
                // all the stale data.
