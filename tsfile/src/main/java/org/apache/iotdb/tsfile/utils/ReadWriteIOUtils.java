...
public class ReadWriteIOUtils {
    ...
    private static final String RETURN_ERROR =
    ...
    public static void writeWithoutSize(
            ByteBuffer byteBuffer, int offset, int len, OutputStream outputStream)
            throws IOException {
        byte[] bytes = byteBuffer.array();
    ...
    /**
     * unlike InputStream.read(bytes), this method makes sure that you can read length bytes or
     * reach to the end of the stream.
     */
    ...
    public static boolean checkIfMagicString(ByteBuffer byteBuffer) {
        byteBuffer.mark();
        boolean res =
                Arrays.equals(readBytes(byteBuffer, magicStringBytes.length), magicStringBytes);
        byteBuffer.reset();
        return res;
    }
