...
public class TsFileSequenceReader implements AutoCloseable {
        ...
        if (metadataIndexNode.getNodeType().equals(MetadataIndexNodeType.INTERNAL_MEASUREMENT)) {
            ...
            for (int i = 0; i < metadataIndexListSize; i++) {
                ...
                if (node.getNodeType().equals(MetadataIndexNodeType.LEAF_DEVICE)) {
                    // if node in next level is LEAF_DEVICE, put all devices in node entry into the
                    // set
                    deviceList.addAll(
     ...
     *     When searching for a device node, return when it is not INTERNAL_DEVICE. Likewise, when
     ...
     *     or else return the nearest MetadataIndexEntry before it (for deeper search)
    ...
    public Chunk readMemChunk(ChunkMetadata metaData) throws IOException {
        ...
        ChunkHeader header =
                readChunkHeader(metaData.getOffsetOfChunkHeader(), chunkHeadSize, false);
        ByteBuffer buffer =
                readChunk(
     ...
     *
     * <p>note that this method loads all the chunks into memory, so it needs to be invoked
     * carefully.
     *
     * @param device name
    ...
    private ByteBuffer readData(long position, int size) throws IOException {
        ...
        if (position < 0) {
            ...
            long actualReadSize =
                    ReadWriteIOUtils.readAsPossible(tsFileInput, buffer, position, size);
     ...
     * position.
     *
     * @param start the start position of data in the tsFileInput, or the current position if
     *     position = -1
    ...
    public long selfCheck(
        ...
        try {
            while ((marker = this.readMarker()) != MetaMarker.SEPARATOR) {
                switch (marker) {
                    case MetaMarker.CHUNK_HEADER:
                        ...
                        // if there is something wrong with a chunk, we will drop the whole
                        // ChunkGroup
                        // as different chunks may be created by the same insertions(sqls), and
                        // partial
                        ...
                        if (newSchema != null) {
                            for (MeasurementSchema tsSchema : measurementSchemaList) {
                                newSchema.putIfAbsent(
                                        new Path(deviceID, tsSchema.getMeasurementId()), tsSchema);
                            }
                        }
                        chunkGroupMetadataList.add(
                                new ChunkGroupMetadata(deviceID, chunkMetadataList));
                        newChunkGroup = true;
            ...
            logger.info(
                    "TsFile {} self-check cannot proceed at position {} "
                            + "recovered, because : {}",
                    file,
                    this.position(),
                    e.getMessage());
