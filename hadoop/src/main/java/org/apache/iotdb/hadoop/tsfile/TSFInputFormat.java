...
public class TSFInputFormat extends FileInputFormat<NullWritable, MapWritable> {
     ...
     *
     * @param configuration
     * @return List of device, if configuration has been set the deviceIds. null, if configuration
     *     has not been set the deviceIds.
     */
    ...
    public static List<TSFInputSplit> getTSFInputSplit(
        ...
        for (FileStatus fileStatus : listFileStatus) {
            ...
            if (length > 0) {
                ...
                logger.info(
                        "The block location information is {}", Arrays.toString(blockLocations));
