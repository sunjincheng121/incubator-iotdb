...
public class TsFileSequenceRead {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (TsFileSequenceReader reader = new TsFileSequenceReader(filename)) {
            ...
            // (the marker) ahead and
            // judge accordingly.
            reader.position(
                    (long) TSFileConfig.MAGIC_STRING.getBytes().length
                ...
                switch (marker) {
                    case MetaMarker.CHUNK_HEADER:
                        ...
                        Decoder valueDecoder =
                                Decoder.getDecoderByType(
                                        header.getEncodingType(), header.getDataType());
                        for (int j = 0; j < header.getNumOfPages(); j++) {
                            ...
                            System.out.println(
                                    "\t\tpoints in the page: " + pageHeader.getNumOfValues());
                            ByteBuffer pageData =
            ...
            for (String device : reader.getAllDevices()) {
                ...
                System.out.println(
                        String.format(
                                "\t[Device]Device %s, Number of Measurements %d",
                ...
                for (Map.Entry<String, List<ChunkMetadata>> serie : seriesMetaData.entrySet()) {
                    ...
                    for (ChunkMetadata chunkMetadata : serie.getValue()) {
                        System.out.println(
                                "\t\tFile offset:" + chunkMetadata.getOffsetOfChunkHeader());
                    }
                }
            }
        }
