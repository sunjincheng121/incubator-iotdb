...
public class IoTDBMultiStatementsIT {
    ...
    private static void insertData() throws ClassNotFoundException, SQLException {
        ...
        try (Connection connection =
            ...
            statement.execute(
                    "CREATE TIMESERIES root.fans.d0.s0 WITH DATATYPE=INT32, ENCODING=RLE");
            statement.execute(
                    "CREATE TIMESERIES root.fans.d0.s1 WITH DATATYPE=INT64, ENCODING=RLE");

            for (int time = 1; time < 10; time++) {

                String sql =
                        String.format(
                                "insert into root.fans.d0(timestamp,s0) values(%s,%s)",
                                time, time % 10);
                statement.execute(sql);
                sql =
                        String.format(
                                "insert into root.fans.d0(timestamp,s1) values(%s,%s)",
                                time, time % 5);
                statement.execute(sql);
            }
    ...
    public void selectAllTest() throws ClassNotFoundException {
        ...
        try (Connection connection =
            ...
            try {
                ...
                // use do-while instead of while because in the previous while loop, we have
                // executed the next function,
                // and the cursor has been moved to the next position, so we should fetch that value
                // first.
                do {
                    StringBuilder builder = new StringBuilder();
                    builder.append(resultSet1.getString(TestConstant.TIMESTAMP_STR))
                ...
                // Although the statement2 has the same sql as statement1, they shouldn't affect
                // each other.
