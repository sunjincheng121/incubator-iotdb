...
public class IoTDBRestartIT {
        ...
        try (Connection connection =
            ...
            try (ResultSet resultSet = statement.getResultSet()) {
                while (resultSet.next()) {
                    String result =
                            resultSet.getString(TIMESTAMP_STR) + "," + resultSet.getString(2);
    ...
    public void testRestartQueryLargerThanEndTime()
        ...
        try (Connection connection =
            ...
            boolean hasResultSet =
                    statement.execute("SELECT s1 FROM root.turbine.d1 where time > 3");
            assertTrue(hasResultSet);
            String[] exp =
                    new String[] {
                        "4,2.0",
    ...
    public void testRestartCompaction()
        ...
        try (Connection connection =
            ...
            assertTrue(hasResultSet);
            String[] exp =
                    new String[] {"1,1.0", "2,1.0", "3,1.0", "4,1.0", "5,1.0", "6,1.0", "7,1.0"};
            int cnt = 0;
            try (ResultSet resultSet = statement.getResultSet()) {
                while (resultSet.next()) {
                    String result =
