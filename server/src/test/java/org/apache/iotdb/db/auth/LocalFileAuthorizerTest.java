...
public class LocalFileAuthorizerTest {
    ...
    public void testAuthorizer() throws AuthException {
        ...
        try {
            ...
            assertEquals(
                    "User user already has INSERT_TIMESERIES on root.laptop.d1", e.getMessage());
        }
        try {
        ...
        try {
            ...
            assertEquals(
                    "User user does not have INSERT_TIMESERIES on root.laptop.d1", e.getMessage());
        }
        try {
            authorizer.deleteUser(user.getName());
            authorizer.revokePrivilegeFromUser(user.getName(), nodeName, 1);
        ...
        try {
            ...
            assertEquals(
                    "Role role already has INSERT_TIMESERIES on root.laptop.d1", e.getMessage());
        }
        ...
        try {
            ...
            assertEquals(
                    "Role role does not have INSERT_TIMESERIES on root.laptop.d1", e.getMessage());
        }
