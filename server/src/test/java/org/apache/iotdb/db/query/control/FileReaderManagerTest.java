    ...
    public void test() throws IOException, InterruptedException {
        ...
        for (int i = 1; i <= MAX_FILE_SIZE; i++) {
            TsFileResource tsFile =
                    new TsFileResource(SystemFileFactory.INSTANCE.getFile(filePath + i));
            Assert.assertTrue(manager.contains(tsFile, false));
        }

        // the code below is not valid because the cacheFileReaderClearPeriod config in this class
        // is not valid
