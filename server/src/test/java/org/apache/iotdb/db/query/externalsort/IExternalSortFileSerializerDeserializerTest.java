...
public class IExternalSortFileSerializerDeserializerTest {
    ...
    public void testFIX_LENGTH() throws IOException {
        ...
        int count = 10000;
        testReadWrite(
                genTimeValuePairs(count, TSDataType.BOOLEAN),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        testReadWrite(
                genTimeValuePairs(count, TSDataType.INT32),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        testReadWrite(
                genTimeValuePairs(count, TSDataType.INT64),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        testReadWrite(
                genTimeValuePairs(count, TSDataType.FLOAT),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        testReadWrite(
                genTimeValuePairs(count, TSDataType.DOUBLE),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        testReadWrite(
                genTimeValuePairs(count, TSDataType.TEXT),
                count,
                rootPath,
                filePath,
                Type.FIX_LENGTH);
        ...
        for (int i = 0; i < count; i++) {
            switch (dataType) {
                case BOOLEAN:
                    timeValuePairs[i] =
                            new TimeValuePair(
                                    i, new TsPrimitiveType.TsBoolean(i % 2 == 0 ? true : false));
                    break;
                case INT32:
                ...
                case DOUBLE:
                    timeValuePairs[i] =
                            new TimeValuePair(i, new TsPrimitiveType.TsDouble(i + 0.12));
                    break;
                case TEXT:
                    timeValuePairs[i] =
                            new TimeValuePair(
                                    i, new TsPrimitiveType.TsBinary(new Binary(String.valueOf(i))));
                    break;
            }
        }
