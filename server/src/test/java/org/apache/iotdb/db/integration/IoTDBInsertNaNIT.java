...
public class IoTDBInsertNaNIT {
    ...
    private static void initCreateSQLStatement() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            sqls.add(
                    String.format(
                            CREATE_TEMPLATE_SQL, "f0", "s" + i + "2f", "FLOAT", "TS_2DIFF", i));
            sqls.add(String.format(CREATE_TEMPLATE_SQL, "d0", "s" + i + "rle", "DOUBLE", "RLE", i));
            sqls.add(
                    String.format(
                            CREATE_TEMPLATE_SQL, "d0", "s" + i + "2f", "DOUBLE", "TS_2DIFF", i));
        }
        for (int i = 0; i < 10; i++) {
                ...
                while (resultSet.next()) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        Assert.assertEquals(
                                ...
                                resultSet.getFloat(
                                        String.format("root.vehicle.%s.%s", "f0", "s" + i + "rle")),
                                DELTA_FLOAT);
                        Assert.assertEquals(
                                MathUtils.roundWithGivenPrecision(Float.parseFloat(VALUE), i),
                                resultSet.getFloat(
                                        String.format("root.vehicle.%s.%s", "f0", "s" + i + "2f")),
                                DELTA_FLOAT);
                        Assert.assertEquals(
                                MathUtils.roundWithGivenPrecision(Double.parseDouble(VALUE), i),
                                resultSet.getDouble(
                                        String.format("root.vehicle.%s.%s", "d0", "s" + i + "rle")),
                                DELTA_DOUBLE);
                        Assert.assertEquals(
                                MathUtils.roundWithGivenPrecision(Double.parseDouble(VALUE), i),
                                resultSet.getDouble(
                                        String.format("root.vehicle.%s.%s", "d0", "s" + i + "2f")),
                                DELTA_DOUBLE);
                    }
                    cnt++;
                }
