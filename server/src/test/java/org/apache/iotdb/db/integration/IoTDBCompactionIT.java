    ...
    public void test() throws SQLException {
        try (Connection connection =
            ...
            for (int i = 0; i < 32; i++) {
                statement.execute(
                        String.format(
                                "INSERT INTO root.compactionTest(timestamp,s1,s2,s3) VALUES (%d,%d,"
                                        + "%d,%d)",
                                i, i + 1, i + 2, i + 3));
            ...
            try (ResultSet resultSet =
                    statement.executeQuery("SELECT * FROM root.compactionTest")) {
                cnt = 0;
    ...
    public void testAppendMergeAfterDeserializeMerge() throws SQLException {
        ...
        try (Connection connection =
            ...
            for (long row = 0; row < 10000; row++) {
                statement.execute(
                        String.format(
                                "INSERT INTO root.compactionTest(timestamp,s1) VALUES (%d,%d)",
                                timestamp, 1));
                if (row % pageSize == 0) {
                    statement.execute("FLUSH");
                }
        ...
        IoTDBDescriptor.getInstance()
                .getConfig()
                .setEnableUnseqCompaction(prevEnableUnseqCompaction);
    }
}
