...
public class SimpleFileVersionControllerTest {
    ...
    public void test() throws IOException {
        ...
        try {
            ...
            assertEquals(
                    SimpleFileVersionController.getSaveInterval(), versionController.currVersion());
            for (int i = 0; i < 150; i++) {
                versionController.nextVersion();
            }
            assertEquals(
                    SimpleFileVersionController.getSaveInterval() + 150,
                    versionController.currVersion());
            versionController = new SimpleFileVersionController(tempFilePath, 1);
            assertEquals(
                    SimpleFileVersionController.getSaveInterval() + 200,
                    versionController.currVersion());
        } finally {
