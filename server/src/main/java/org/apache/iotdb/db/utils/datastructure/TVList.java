...
public abstract class TVList {
    ...
    public static long tvListArrayMemSize(TSDataType type) {
        ...
        // value size
        size += PrimitiveArrayManager.ARRAY_SIZE * type.getDataTypeSize();
        return size;
    }
    ...
    /**
     * The arrays for sorting are not including in write memory now, the memory usage is considered
     * as temporary memory.
     */
    ...
    private class Ite implements IPointReader {
        ...
        /**
         * because TV list may be share with different query, each iterator has to record it's own
         * size
         */
