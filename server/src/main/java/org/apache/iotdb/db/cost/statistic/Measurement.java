...
public class Measurement implements MeasurementMBean, IService {
    ...
    /**
     * the num of operation that execution time falls into time range of BUCKET_IN_MS. The outer
     * array is each operation, the inner array is each time range in BUCKET_IN_MS.
     */
    ...
    public boolean addOperationLatency(Operation op, long startTime) {
        if (isEnableStat && operationSwitch.get(op.getName())) {
            return operationLatenciesQueue[op.ordinal()].put(
            ...
            if (consumeFuture != null && !consumeFuture.isCancelled()) {
                ...
                consumeFuture =
                        service.scheduleWithFixedDelay(
                                new Measurement.DisplayRunnable(),
                                20,
                                displayIntervalInMs,
    ...
    private void showMeasurements() {
        ...
        String head =
                String.format(
                        "%-45s%-25s%-25s%-25s", "OPERATION", "COUNT", "TOTAL_TIME", "AVG_TIME");
        logger.info(head);
        for (Operation operation : Operation.values()) {
            ...
            String item =
                    String.format(
                            "%-45s%-25s%-25s%-25s", operation.name, cnt + "", totalInMs + "", avg);
            logger.info(item);
        }
        logger.info(
                "==========================================OPERATION HISTOGRAM====================================================");
        ...
        for (Operation operation : Operation.values()) {
            ...
            for (int i = 0; i < BUCKET_SIZE; i++) {
                String avg =
                        String.format(
                                "%.2f",
                                (operationHistogram[operation.ordinal()][i] / (cnt + 1e-9) * 100));
