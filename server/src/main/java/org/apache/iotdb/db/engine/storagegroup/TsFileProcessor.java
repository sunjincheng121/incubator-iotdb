...
public class TsFileProcessor {
    ...
    private void checkMemCostAndAddToTspInfo(InsertRowPlan insertRowPlan)
        ...
        for (int i = 0; i < insertRowPlan.getDataTypes().length; i++) {
            ...
            if (workMemTable.checkIfChunkDoesNotExist(
                    deviceId, insertRowPlan.getMeasurements()[i])) {
                ...
                int currentChunkPointNum =
                        workMemTable.getCurrentChunkPointNum(
                                deviceId, insertRowPlan.getMeasurements()[i]);
                memTableIncrement +=
    ...
    private void updateMemoryInfo(
        ...
        if (storageGroupInfo.needToReportToSystem()) {
            ...
            try {
                ...
                tsFileProcessorInfo.releaseTSPMemCost(
                        unsealedResourceIncrement + chunkMetadataIncrement);
    ...
    /**
     * Delete data which belongs to the timeseries `deviceId.measurementId` and the timestamp of
     * which <= 'timestamp' in the deletion. <br>
     *
    ...
    public void deleteDataInMemory(Deletion deletion, Set<PartialPath> devicePaths) {
        ...
        try {
            ...
            // flushing memTables are immutable, only record this deletion in these memTables for
            // query
    ...
    void asyncClose() {
        ...
        try {
            ...
            // when a flush thread serves this TsFileProcessor (because the processor is submitted
            // by
            ...
            // To ensure there must be a flush thread serving this processor after the field
            // `shouldClose`
            // is set true, we need to generate a NotifyFlushMemTable as a signal task and submit it
            // to
            // the FlushManager.
            ...
            try {
                // When invoke closing TsFile after insert data to memTable, we shouldn't flush
                // until invoke
     ...
     * queue, set the current working memtable as null and then register the tsfileProcessor into
     * the flushManager again.
     */
            ...
            if (!readOnlyMemChunks.isEmpty() || !chunkMetadataList.isEmpty()) {
                tsfileResourcesForQuery.add(
                        new TsFileResource(
                                ...
                                tsFileResource.getStartTimes(),
                                tsFileResource.getEndTimes(),
                                readOnlyMemChunks,
                                chunkMetadataList,
                                tsFileResource));
            }
