...
public class FileReaderManager implements IService {
    ...
    /**
     * the key of unclosedFileReaderMap is the file path and the value of unclosedFileReaderMap is
    ...
    private void clearUnUsedFilesInFixTime() {
        ...
        long examinePeriod =
                IoTDBDescriptor.getInstance().getConfig().getCacheFileReaderClearPeriod();
        ...
        while (iterator.hasNext()) {
            ...
            if (refAtom != null && refAtom.get() == 0) {
                try {
                    ...
                    logger.error(
                            "Can not close TsFileSequenceReader {} !", reader.getFileName(), e);
                }
                iterator.remove();
    ...
    /**
     * Get the reader of the file(tsfile or unseq tsfile) indicated by filePath. If the reader
     * already exists, just get it from closedFileReaderMap or unclosedFileReaderMap depending on
     * isClosing . Otherwise a new reader will be created and cached.
     *
    ...
    /**
     * Increase the reference count of the reader specified by filePath. Only when the reference
     * count of a reader equals zero, the reader can be closed and removed.
     */
