    ...
    public static void main(String[] args) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(outFile))) {
            ...
            try (TsFileSequenceReader reader = new TsFileSequenceReader(filename)) {
                ...
                printlnBoth(
                        pw, String.format("%20d", 0) + "|\t[magic head] " + reader.readHeadMagic());
                printlnBoth(
                        pw,
                ...
                for (ChunkGroupMetadata chunkGroupMetadata : allChunkGroupMetadata) {
                    ...
                    ChunkGroupFooter chunkGroupFooter =
                            reader.readChunkGroupFooter(chunkEndPos, false);
                    printlnBoth(pw, String.format("%20s", "") + "|\t\t[marker] 0");
                    printlnBoth(
                            pw,
                            String.format("%20s", "")
                                    + "|\t\t[deviceID] "
                                    + chunkGroupFooter.getDeviceID());
                    printlnBoth(
                            pw,
                            String.format("%20s", "")
                                    + "|\t\t[dataSize] "
                    ...
                    printlnBoth(
                            ...
                            String.format("%20s", "")
                                    + "|\t\t[num of chunks] "
                                    + chunkGroupFooter.getNumberOfChunks());
                    printlnBoth(
                            pw,
                            str1.toString()
                                    + "\t[Chunk Group] of "
                    ...
                    if (versionMap.containsKey(
                            chunkEndPos + chunkGroupFooter.getSerializedSize())) {
                        printlnBoth(
                                pw,
                                String.format(
                                                "%20s",
                ...
                for (Map.Entry<String, List<TimeseriesMetadata>> entry :
                    ...
                    for (TimeseriesMetadata seriesMetadata : seriesMetadataList) {
                        timeseriesMetadataMap.put(
                                ...
                                new Pair<>(
                                        new Path(device, seriesMetadata.getMeasurementId()),
                                        seriesMetadata));
                    }
                }
                ...
                printlnBoth(
                        pw,
                        String.format("%20s", reader.getFileMetadataPos()) + "|\t[TsFileMetadata]");
                printlnBoth(
                        pw,
                        String.format("%20s", "")
                                + "|\t\t[num of devices] "
                ...
                printlnBoth(
                        ...
                        String.format("%20s", "")
                                + "|\t\t[totalChunkNum] "
                                + tsFileMetaData.getTotalChunkNum());
                printlnBoth(
                        pw,
                        String.format("%20s", "")
                                + "|\t\t[invalidChunkNum] "
                                + tsFileMetaData.getInvalidChunkNum());
