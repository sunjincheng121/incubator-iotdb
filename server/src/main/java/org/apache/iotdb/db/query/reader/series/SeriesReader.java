...
public class SeriesReader {
        ...
        unSeqPageReaders =
                new PriorityQueue<>(
                        orderUtils.comparingLong(
                                versionPageReader ->
                                        orderUtils.getOrderTime(
                                                versionPageReader.getStatistics())));
    }
    ...
    boolean isFileOverlapped() throws IOException {
        ...
        return !seqTimeSeriesMetadata.isEmpty()
                        && orderUtils.isOverlapped(
                                fileStatistics, seqTimeSeriesMetadata.get(0).getStatistics())
    ...
    private void unpackAllOverlappedTimeSeriesMetadataToCachedChunkMetadata(
        ...
        while (!seqTimeSeriesMetadata.isEmpty()
                && orderUtils.isOverlapped(
                        endpointTime, seqTimeSeriesMetadata.get(0).getStatistics())) {
    ...
    private void unpackOneTimeSeriesMetadata(TimeseriesMetadata timeSeriesMetadata)
        ...
        chunkMetadataList.forEach(
                chunkMetadata -> chunkMetadata.setSeq(timeSeriesMetadata.isSeq()));
        ...
        if (IoTDBDescriptor.getInstance().getConfig().isEnablePerformanceTracing()) {
            ...
            queryResourceManager
                    ...
                    .compute(
                            context.getQueryId(),
                            (k, v) -> v == null ? totalChunkSize : v + totalChunkSize);
        }
    ...
    boolean isChunkOverlapped() throws IOException {
        ...
        return !cachedChunkMetadata.isEmpty()
                && orderUtils.isOverlapped(
                        chunkStatistics, cachedChunkMetadata.peek().getStatistics());
    }
    ...
    boolean hasNextPage() throws IOException {
        ...
        // make sure firstPageReader won't be null while cachedPageReaders has more cached page
        // readers
    ...
    private void unpackAllOverlappedChunkMetadataToCachedPageReaders(
            long endpointTime, boolean init) throws IOException {
        while (!cachedChunkMetadata.isEmpty()
                && orderUtils.isOverlapped(
                        endpointTime, cachedChunkMetadata.peek().getStatistics())) {
                        ...
                        pageReader -> {
                            if (chunkMetaData.isSeq()) {
                                ...
                                if (orderUtils.getAscending()) {
                                    seqPageReaders.add(
                                            new VersionPageReader(
                                                    chunkMetaData.getVersion(), pageReader, true));
                                } else {
                                    seqPageReaders.add(
                                            0,
                                            new VersionPageReader(
                                ...
                                unSeqPageReaders.add(
                                        new VersionPageReader(
                                                chunkMetaData.getVersion(), pageReader, false));
                            }
                        });
    }
    ...
    boolean isPageOverlapped() throws IOException {
        ...
        return !unSeqPageReaders.isEmpty()
                && orderUtils.isOverlapped(
                        firstPageStatistics, unSeqPageReaders.peek().getStatistics());
    }
    ...
    private void unpackAllOverlappedUnseqPageReadersToMergeReader(long endpointTime)
        ...
        while (!unSeqPageReaders.isEmpty()
                && orderUtils.isOverlapped(
                        endpointTime, unSeqPageReaders.peek().data.getStatistics())) {
    ...
    private LinkedList<TsFileResource> sortUnSeqFileResources(
        ...
        return tsFileResources.stream()
                .sorted(
                        orderUtils.comparingLong(
                                tsFileResource -> orderUtils.getOrderTime(tsFileResource)))
     ...
     * the chunks at once, which may cause OOM, so we can only unpack one file at a time when
     * needed. This approach is likely to be ubiquitous, but it keeps the system running smoothly
     */
    @SuppressWarnings("squid:S3776") // Suppress high Cognitive Complexity warning
    ...
    public interface TimeOrderUtils {
        ...
        long getCurrentEndPoint(
                Statistics<? extends Object> seqStatistics,
    ...
    class DescTimeOrderUtils implements TimeOrderUtils {
        ...
        public <T> Comparator<T> comparingLong(ToLongFunction<? super T> keyExtractor) {
            ...
            return (Comparator<T> & Serializable)
                    (c1, c2) ->
                            Long.compare(
                                    keyExtractor.applyAsLong(c2), keyExtractor.applyAsLong(c1));
        }
