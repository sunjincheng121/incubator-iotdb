...
public class IoTDBConfig {
     ...
     * If set this parameter to 0 it means call outputStream.force(true) after every each insert
     */
    ...
    private String tracingDir =
            DEFAULT_BASE_DIR + File.separator + IoTDBConstant.TRACING_FOLDER_NAME;
     ...
     * one {@linkplain org.apache.iotdb.db.index.IndexFileProcessor IndexFileProcessor} share a
     * total common buffer size. With the memory-control mechanism, the occupied memory of all raw
     * data and index structures will be counted. If the memory buffer size reaches this threshold,
     * the indexes will be flushed to the disk file. As a result, data in one series may be divided
     * into more than one part and indexed separately.
     */
    ...
    /**
     * Work when tsfile_manage_strategy is level_strategy. When merge point number reaches this,
     * merge the files to the last level. During a merge, if a chunk with less number of chunks than
     * this parameter, the chunk will be merged with its succeeding chunks even if it is not
     * overflowed, until the merged chunks reach this threshold and the new chunk will be flushed.
     */
     ...
     * When the num of files in one level exceeds this, the files in this level will merge to one
     * and put to upper level.
     */
    ...
    /**
     * Works when compaction_strategy is LEVEL_COMPACTION. The max ujseq file num of each level.
     * When the num of files in one level exceeds this, the files in this level will merge to one
     * and put to upper level.
     */
    ...
    /**
     * register time series as which type when receiving the Literal NaN. Values can be DOUBLE,
     * FLOAT or TEXT
     */
     ...
     * selection will be used as final selection. Unit: millis. When < 0, it means time is
     * unbounded.
     */
    private long mergeFileSelectionTimeBudget = 30 * 1000L;

    /**
     * When set to true, if some crashed merges are detected during system rebooting, such merges
     * will be continued, otherwise, the unfinished parts of such merges will not be continued while
     * the finished parts still remain as they are.
     */
    private boolean continueMergeAfterReboot = false;

    /**
     * A global merge will be performed each such interval, that is, each storage group will be
     * merged (if proper merge candidates can be found). Unit: second.
     */
    ...
    /**
     * How many thread will be set up to perform compaction, 10 by default. Set to 1 when less than
     * or equal to 0.
     */
    ...
    /**
     * Threshold interval time of MTree modification. If the last modification time is less than
     * this threshold, MTree snapshot will not be created. Unit: second. Default: 1 hour(3600
     * seconds)
     */
