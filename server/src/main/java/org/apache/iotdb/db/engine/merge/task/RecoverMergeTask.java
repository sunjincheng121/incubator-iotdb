...
public class RecoverMergeTask extends MergeTask {
        ...
        switch (status) {
            ...
            default:
                throw new UnsupportedOperationException(
                        taskName + " found unrecognized status " + status);
        }
        if (logger.isInfoEnabled()) {
            logger.info(
                    "{} merge recovery ends after {}ms",
                    taskName,
                    (System.currentTimeMillis() - startTime));
        }
    }
    ...
    private void resumeAfterFilesLogged(boolean continueMerge) throws IOException {
        if (continueMerge) {
            ...
            MergeFileTask mergeFileTask =
                    new MergeFileTask(
                            taskName, mergeContext, mergeLogger, resource, resource.getSeqFiles());
    ...
    private void resumeAfterAllTsMerged(boolean continueMerge) throws IOException {
        if (continueMerge) {
            ...
            // NOTICE: although some of the seqFiles may have been truncated in last merge, we do
            // not
    ...
    private void calculateConcurrentSeriesNum() throws IOException {
        ...
        for (TsFileResource seqFile : resource.getSeqFiles()) {
            ...
            long maxChunkNum = chunkNums[1];
            long fileMetaSize =
                    MergeUtils.getFileMetaSize(seqFile, resource.getFileReader(seqFile));
    ...
    private void recoverChunkCounts(
        ...
        List<ChunkMetadata> mergeFileChunks =
                mergeFileWriter.getVisibleMetadataList(
                        path.getDevice(), path.getMeasurement(), null);
        mergeContext
                .getMergedChunkCnt()
                .compute(
                        tsFileResource,
        ...
        mergeContext
                ...
                .compute(
                        tsFileResource,
                        (k, v) -> v == null ? finalUnmergedCnt : v + finalUnmergedCnt);
    }
