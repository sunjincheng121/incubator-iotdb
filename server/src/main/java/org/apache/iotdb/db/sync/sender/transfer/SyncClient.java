...
public class SyncClient implements ISyncClient {
     ...
     * location is recorded once after each synchronization task for the next synchronization task
     * to use.
     */
    ...
    public void startMonitor() {
        executorService.scheduleWithFixedDelay(
                () -> {
                    if (syncStatus) {
                        logger.info(
                                "Sync process for receiver {} is in execution!",
                                config.getSyncReceiverName());
                    }
                },
    ...
    public void establishConnection(String serverIp, int serverPort)
        ...
        transport =
                RpcTransportFactory.INSTANCE.getTransport(
                        new TSocket(serverIp, serverPort, TIMEOUT_MS));
    ...
    private String getOrCreateUUID(File uuidFile) throws IOException {
        ...
        if (!uuidFile.exists()) {
            ...
            try (BufferedReader bf =
                    new BufferedReader((new FileReader(uuidFile.getAbsolutePath())))) {
                uuid = bf.readLine();
            } catch (IOException e) {
    ...
    private boolean tryToSyncSchema() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(getSchemaLogFile()));
                ByteArrayOutputStream bos =
                        new ByteArrayOutputStream(SyncConstant.DATA_CHUNK_SIZE)) {
            schemaFileLinePos = 0;
            String line;
            ...
            if (bos.size() != 0) {
                ...
                if (status.code != SUCCESS_CODE) {
                    logger.error(
                            "Receiver failed to receive metadata because {}, retry.", status.msg);
                    return false;
                }
            }
    ...
    private boolean checkDigestForSchema(String digestOfSender) throws TException {
        ...
        if (status.code == SUCCESS_CODE && digestOfSender.equals(status.msg)) {
            ...
            logger.error(
                    "Digest check of schema file {} failed, retry",
        ...
        try {
            ...
            for (Entry<String, Set<Long>> entry : allSG.entrySet()) {
                ...
                try {
                    ...
                    if (status.code != SUCCESS_CODE) {
                        throw new SyncConnectionException(
                                "Unable init receiver because " + status.msg);
                    }
    ...
    public void syncDataFilesInOneGroup(String sgName, Long timeRangeId, Set<File> toBeSyncFiles)
        ...
        for (File tsfile : toBeSyncFiles) {
            ...
            try {
                ...
                syncSingleFile(
                        new File(snapshotFile.getAbsolutePath() + TsFileResource.RESOURCE_SUFFIX));
                syncSingleFile(snapshotFile);
                lastLocalFilesMap.get(sgName).get(timeRangeId).add(tsfile);
                syncLog.finishSyncTsfile(tsfile);
                logger.info(
                        "Task of synchronization has completed {}/{}.", cnt, toBeSyncFiles.size());
            } catch (IOException e) {
                logger.info(
        ...
        try {
            ...
            while (true) {
                ...
                try (FileInputStream fis = new FileInputStream(snapshotFile);
                        ByteArrayOutputStream bos =
                                new ByteArrayOutputStream(SyncConstant.DATA_CHUNK_SIZE)) {
                    while ((dataLength = fis.read(buffer))
                            != -1) { // cut the file into pieces to send
                ...
                if (status.code == SUCCESS_CODE && digestOfSender.equals(status.msg)) {
                    logger.info(
                            "Receiver has received {} successfully.",
                            snapshotFile.getAbsoluteFile());
                    break;
                } else {
                    logger.error(
                            "Digest check of tsfile {} failed, retry",
                            snapshotFile.getAbsoluteFile());
                }
            }
    ...
    private File getSchemaPosFile() {
        return new File(
                IoTDBDescriptor.getInstance().getConfig().getSyncDir(),
