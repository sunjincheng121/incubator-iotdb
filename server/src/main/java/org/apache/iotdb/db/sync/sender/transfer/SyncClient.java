...
public class SyncClient implements ISyncClient {
  ...
  private File getSchemaLogFile() {
    return new File(
        IoTDBDescriptor.getInstance().getConfig().getSchemaDir(), MetadataConstant.METADATA_LOG);
  }
  ...
  private File getUuidFile() {
    return new File(
        IoTDBDescriptor.getInstance().getConfig().getSyncDir(), SyncConstant.UUID_FILE_NAME);
  }
