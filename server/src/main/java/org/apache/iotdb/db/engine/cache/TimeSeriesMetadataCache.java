    ...
    private TimeSeriesMetadataCache() {
        ...
        lruCache =
                new LRULinkedHashMap<TimeSeriesMetadataCacheKey, TimeseriesMetadata>(
                    ...
                    @Override
                    protected long calEntrySize(
                            TimeSeriesMetadataCacheKey key, TimeseriesMetadata value) {
                        long currentSize;
                        if (count < 10) {
                            currentSize =
            ...
            synchronized (key.device.intern()) {
                ...
                if (timeseriesMetadata != null) {
                    ...
                    if (bloomFilter != null
                        ...
                        if (config.isDebugOn()) {
                            DEBUG_LOGGER.info(
                                    "TimeSeries meta data " + key + " is filter by bloomFilter!");
                        }
                        return null;
                    }
                    List<TimeseriesMetadata> timeSeriesMetadataList =
                            reader.readTimeseriesMetadata(key.device, allSensors);
