...
public class SystemInfo {
    ...
    private void checkSystemToInvokeFlush() {
        if (totalSgMemCost >= FLUSH_THERSHOLD && totalSgMemCost < REJECT_THERSHOLD) {
            logger.debug(
                    "Some sg memory released but still exceeding flush proportion, call flush.");
            if (rejected) {
    ...
    /**
     * Order all tsfileProcessors in system by memory cost of actual data points in memtable. Mark
    ...
    private List<TsFileProcessor> getTsFileProcessorsToFlush() {
        PriorityQueue<TsFileProcessor> tsps =
                new PriorityQueue<>(
                        (o1, o2) ->
                                Long.compare(
                                        o2.getWorkMemTableRamCost(), o1.getWorkMemTableRamCost()));
