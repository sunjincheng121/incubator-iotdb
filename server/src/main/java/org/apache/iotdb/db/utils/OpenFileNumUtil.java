...
public class OpenFileNumUtil {
    ...
    private static int getIotdbPid() {
        ...
        if (osName.startsWith(LINUX_OS_NAME) || osName.startsWith(MAC_OS_NAME)) {
            try {
                ...
                BufferedReader in1 =
                        new BufferedReader(new InputStreamReader(pro1.getInputStream()));
                String line;
     ...
     * number of TsFile; UNSEQUENCE_FILE_OPEN_NUM is the current open file number of unsequence
     * file; WAL_OPEN_FILE_NUM is the current open file number of WAL file; METADATA_OPEN_FILE_NUM
     * is the current open file number of metadata; DIGEST_OPEN_FILE_NUM is the current open file
     * number of fileNodeDir; SOCKET_OPEN_FILE_NUM is the current open socket connection of IoTDB
     * service process.
     *
    ...
    private static EnumMap<OpenFileNumStatistics, Integer> getOpenFile(int pid) {
        EnumMap<OpenFileNumStatistics, Integer> resultMap =
                new EnumMap<>(OpenFileNumStatistics.class);
