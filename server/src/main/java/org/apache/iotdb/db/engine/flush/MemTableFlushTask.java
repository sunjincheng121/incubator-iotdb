...
public class MemTableFlushTask {
    ...
    public MemTableFlushTask(
        ...
        logger.debug(
                "flush task of Storage group {} memtable {} is created ",
                storageGroup,
                memTable.getVersion());
    }
    ...
    public void syncFlushMemTable() throws ExecutionException, InterruptedException, IOException {
        logger.info(
                "The memTable size of SG {} is {}, the avg series points num in chunk is {} ",
                storageGroup,
                memTable.memSize(),
    ...
    private Runnable encodingTask =
            new Runnable() {
                ...
                public void run() {
                    ...
                    logger.debug(
                            "Storage group {} memtable {}, starts to encoding data.",
                            storageGroup,
                            memTable.getVersion());
                    while (true) {
                        ...
                        if (task == null) {
                            ...
                            try {
                                ...
                                logger.error(
                                        "Storage group {} memtable {}, encoding task is interrupted.",
                                        storageGroup,
                                        memTable.getVersion(),
                                        e);
                            ...
                            if (task instanceof StartFlushGroupIOTask
                                    || task instanceof EndChunkGroupIoTask) {
                                ...
                                IChunkWriter seriesWriter =
                                        new ChunkWriterImpl(encodingMessage.right);
                                writeOneSeries(
                                        encodingMessage.left,
                                        seriesWriter,
                    ...
                    noMoreIOTask = true;
                    logger.debug(
                            "Storage group {}, flushing memtable {} into disk: Encoding data cost "
                                    + "{} ms.",
                            storageGroup,
                            memTable.getVersion(),
                            memSerializeTime);
    ...
    private Runnable ioTask =
            () -> {
                ...
                logger.debug(
                        "Storage group {} memtable {}, start io.",
                        storageGroup,
                        memTable.getVersion());
                while (true) {
                    if (noMoreIOTask) {
                    ...
                    if (ioMessage == null) {
                        ...
                        try {
                            TimeUnit.MILLISECONDS.sleep(10);
                                ...
                                InterruptedException e) {
                            logger.error(
                                    "Storage group {} memtable {}, io task is interrupted.",
                                    storageGroup,
                                    memTable.getVersion());
                            // generally it is because the thread pool is shutdown so the task
                            // should be aborted
                        ...
                        try {
                            if (ioMessage instanceof StartFlushGroupIOTask) {
                                this.writer.startChunkGroup(
                                        ((StartFlushGroupIOTask) ioMessage).deviceId);
                ...
                logger.debug(
                        "flushing a memtable {} in storage group {}, io cost {}ms",
                        memTable.getVersion(),
                        storageGroup,
                        ioTime);
            };
