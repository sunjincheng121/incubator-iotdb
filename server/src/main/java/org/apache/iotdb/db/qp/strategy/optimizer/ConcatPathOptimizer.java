...
public class ConcatPathOptimizer implements ILogicalOptimizer {
    ...
    public Operator transform(Operator operator, int maxDeduplicatedPathNum)
        ...
        if (operator instanceof QueryOperator) {
            if (!((QueryOperator) operator).isAlignByDevice()
                ...
                // ALIGN_BY_DEVICE leaves the 1) concat, 2) remove star, 3) slimit tasks to the next
                // phase,
    ...
    private void checkAggrOfSelectOperator(SelectOperator selectOperator)
        ...
        if (!selectOperator.getAggregations().isEmpty()
                && selectOperator.getSuffixPaths().size()
                        != selectOperator.getAggregations().size()) {
            throw new LogicalOptimizeException(
    ...
    private FilterOperator concatFilter(
        ...
        if (noStarPaths.size() == 1) {
            ...
            // "select s1 from root.car.d1, root.car.d2 where root.car.d1.s1 > 10 and root.car.d2.s1
            // > 10"
            // Note that,
            // two fork tree has to be maintained while removing stars in paths for
            // DnfFilterOptimizer
            // requirement.
    ...
    private FilterOperator constructBinaryFilterTreeWithAnd(
            List<PartialPath> noStarPaths, FilterOperator operator)
            throws LogicalOptimizeException {
    ...
    protected Pair<List<PartialPath>, Integer> removeWildcard(
