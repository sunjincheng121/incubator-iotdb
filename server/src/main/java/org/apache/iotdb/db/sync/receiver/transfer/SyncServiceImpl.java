...
public class SyncServiceImpl implements SyncService.Iface {
        ...
        try {
            ...
            syncLog.set(
                    new SyncReceiverLogger(
                            new File(syncFolderPath.get(), SyncConstant.SYNC_LOG_NAME)));
            return getSuccessResult();
        ...
        try {
            syncLog.get()
                    .finishSyncDeletedFileName(
                            new File(
                                    getSyncDataPath(),
                                    currentSG.get() + File.separatorChar + fileName));
            ...
            return getErrorResult(
                    String.format(
                            "Can not sync deleted file %s because %s", fileName, e.getMessage()));
        }
        return getSuccessResult();
    }
    ...
    public SyncStatus initSyncData(String filename) throws TException {
        try {
            ...
            return getErrorResult(
                    String.format(
                            "Can not init sync resource for file %s because %s",
    ...
    public SyncStatus checkDataDigest(String digestOfSender) throws TException {
        ...
        try {
            ...
            logger.error(
                    "Can not check data digest for file {}",
                    currentFile.get().getAbsoluteFile(),
                    e);
            return getErrorResult(
                    String.format(
            ...
            logger.error(
                    "Device owner has conflicts, skip all other tsfiles in the sg {}.",
                    currentSG.get());
    ...
    private void loadMetadata() {
        ...
        if (currentFile.get().exists()) {
            try (BufferedReader br =
                    new BufferedReader(new java.io.FileReader(currentFile.get()))) {
                ...
                while ((metadataOperation = br.readLine()) != null) {
                    try {
                        ...
                        logger.error(
                                "Can not operate metadata operation {} ", metadataOperation, e);
                    }
                }
            } catch (IOException e) {
    ...
    public SyncStatus endSync() throws TException {
        try {
            ...
            if (loader != null) {
                ...
                return getErrorResult(
                        String.format(
                                "File Loader of the storage group %s is null", currentSG.get()));
            }
