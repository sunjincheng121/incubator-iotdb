...
public class MTree implements Serializable {
        ...
        synchronized (this) {
            ...
            MeasurementMNode leaf =
                    new MeasurementMNode(
                            cur, leafName, alias, dataType, encoding, compressor, props);
     ...
     */
    MNode getDeviceNodeWithAutoCreating(PartialPath deviceId, int sgLevel)
            throws MetadataException {
     ...
     *
     * <p>e.g., path = root.a.b.sg. if nor a and b is StorageGroupMNode and sg is a
     * StorageGroupMNode path is a storage group
     *
     * @param path path
     * @apiNote :for cluster
     */
     ...
     * device], throw exception Get storage group node, if the give path is not a storage group,
     * throw exception
     */
     ...
     * other words, this path is part of the result storage groups. 2. This path is a full path and
     * we use this method to find its belonged storage group. When prefixOnly is set to true,
     * storage group paths in 1 is only added into result, otherwise, both 1 and 2 are returned.
     *
     ...
     *
     * @param prefixPath a prefix path or a full path, may contain '*'.
     * @return Pair.left contains all the satisfied paths Pair.right means the current offset or
     *     zero if we don't set offset.
     */
            ...
            if (next != null) {
                cnt +=
                        getStorageGroupCount(
                                next, nodes, idx + 1, parent + node.getName() + PATH_SEPARATOR);
            }
        } else {
            for (MNode child : node.getChildren().values()) {
                cnt +=
    ...
    private void findPath(
        ...
        if (node instanceof MeasurementMNode && nodes.length <= idx) {
            ...
            tsRow[6] =
                    needLast
                            ? String.valueOf(
                                    getLastTimeStamp((MeasurementMNode) node, queryContext))
                            : null;
        ...
        for (Entry<String, MNode> entry : mNode.getChildren().entrySet()) {
            ...
            if (child instanceof StorageGroupMNode) {
                ...
                if (depth >= nodes.length - 1 && currNode.equals(PATH_WILDCARD)) {
                    // the we find the sg at the last node and the last node is a wildcard (find
                    // "root
