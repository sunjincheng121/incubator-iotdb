...
public class QueryResourceManager {
        ...
        totalFreeMemoryForRead =
                new AtomicLong(
                        IoTDBDescriptor.getInstance()
                                .getConfig()
                                .getAllocateMemoryForReadWithoutCache());
    }
    ...
    public int getMaxDeduplicatedPathNum(int fetchSize) {
        return Math.min(
                (int) ((totalFreeMemoryForRead.get() / fetchSize) / POINT_ESTIMATED_SIZE),
            ...
            if (config.isEnablePerformanceTracing()) {
                ...
                if (chunkNumMap.get(queryId) != null && chunkSizeMap.get(queryId) != null) {
                    TracingManager.getInstance()
                            .writeChunksInfo(
                                    queryId,
                                    chunkNumMap.remove(queryId),
                                    chunkSizeMap.remove(queryId));
                }
                if (isprinted) {
