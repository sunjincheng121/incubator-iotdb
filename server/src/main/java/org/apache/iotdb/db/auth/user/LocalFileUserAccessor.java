...
public class LocalFileUserAccessor implements IUserAccessor {
        ...
        try (DataInputStream dataInputStream =
            ...
            for (int i = 0; i < privilegeNum; i++) {
                pathPrivilegeList.add(
                        IOUtils.readPathPrivilege(
                                dataInputStream, STRING_ENCODING, strBufferLocal));
            }
    ...
    public void saveUser(User user) throws IOException {
        ...
        try (BufferedOutputStream outputStream =
            ...
            try {
                IOUtils.writeString(
                        outputStream, user.getName(), STRING_ENCODING, encodingBufferLocal);
                IOUtils.writeString(
                        outputStream, user.getPassword(), STRING_ENCODING, encodingBufferLocal);
    ...
    public List<String> listAllUsers() {
        ...
        if (names != null) {
            ...
            for (String fileName : names) {
                set.add(
                        fileName.replace(IoTDBConstant.PROFILE_SUFFIX, "")
                                .replace(TEMP_SUFFIX, ""));
            }
            retList.addAll(set);
        }
