...
public class InsertRowPlan extends InsertPlan {
    ...
    public InsertRowPlan(InsertRowPlan another) {
        ...
        System.arraycopy(
                another.measurements, 0, this.measurements, 0, another.measurements.length);
    ...
    private void putValues(DataOutputStream outputStream)
            throws QueryProcessException, IOException {
        for (int i = 0; i < values.length; i++) {
            if (measurements[i] == null) {
                continue;
            }
            // types are not determined, the situation mainly occurs when the plan uses string
            // values
