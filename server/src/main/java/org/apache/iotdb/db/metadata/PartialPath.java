...
public class PartialPath extends Path implements Comparable<Path> {
    ...
    /**
     * Construct the PartialPath using a String, will split the given String into String[] E.g.,
     * path = "root.sg.\"d.1\".\"s.1\"" nodes = {"root", "sg", "\"d.1\"", "\"s.1\""}
     *
    ...
    public PartialPath alterPrefixPath(PartialPath prefixPath) {
        String[] newNodes =
                Arrays.copyOf(nodes, Math.max(nodes.length, prefixPath.getNodeLength()));
     ...
     * without wildcards. e.g. "root.sg.device.*" matches path "root.sg.device.s1" whereas it does
     * not match "root.sg.device" and "root.sg.vehicle.s1"
     *
