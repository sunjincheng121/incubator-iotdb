...
public class MergeSingleFilterOptimizer implements IFilterOptimizer {
    ...
    private void checkInnerFilterLen(List<FilterOperator> children)
     ...
     * if all recursive children of this node have same seriesPath, set this node to single node,
     * and return the same seriesPath, otherwise, throw exception;
     *
     * @param filter - children is not empty.
     * @return - if all recursive children of this node have same seriesPath, set this node to
     *     single node, and return the same seriesPath, otherwise, throw exception;
     */
    private PartialPath mergeSamePathFilter(FilterOperator filter) throws LogicalOptimizeException {
        ...
        for (int i = 1; i < children.size(); i++) {
            ...
            // if one of children differs from others or is not single node(seriesPath = null),
            // filter's
            // seriesPath is null
        ...
        // sort paths of BasicFunction by their single seriesPath. We don't sort children on
        // non-leaf
        // layer.
    ...
    private int mergeSingleFilters(List<FilterOperator> ret, FilterOperator filter) {
        ...
        for (firstNonSingleIndex = 0;
                firstNonSingleIndex < children.size();
                firstNonSingleIndex++) {
