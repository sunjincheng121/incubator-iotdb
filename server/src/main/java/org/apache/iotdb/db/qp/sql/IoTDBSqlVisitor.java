...
public class IoTDBSqlVisitor extends SqlBaseBaseVisitor<Operator> {
    ...
    public void parseIndexWithClause(
            IndexWithClauseContext ctx, CreateIndexOperator createIndexOp) {
        IndexType indexType;
        try {
    ...
    public Operator visitCreateUser(CreateUserContext ctx) {
        AuthorOperator authorOperator =
                new AuthorOperator(
                        SQLConstant.TOK_AUTHOR_CREATE, AuthorOperator.AuthorType.CREATE_USER);
    ...
    public Operator visitDropUser(DropUserContext ctx) {
        AuthorOperator authorOperator =
                new AuthorOperator(
                        SQLConstant.TOK_AUTHOR_DROP, AuthorOperator.AuthorType.DROP_USER);
        authorOperator.setUserName(ctx.ID().getText());
        return authorOperator;
    }
    ...
    public Operator visitCreateRole(CreateRoleContext ctx) {
        AuthorOperator authorOperator =
                new AuthorOperator(
                        SQLConstant.TOK_AUTHOR_CREATE, AuthorOperator.AuthorType.CREATE_ROLE);
    ...
    public Operator visitDropRole(DropRoleContext ctx) {
        AuthorOperator authorOperator =
                new AuthorOperator(
                        SQLConstant.TOK_AUTHOR_DROP, AuthorOperator.AuthorType.DROP_ROLE);
        authorOperator.setRoleName(ctx.ID().getText());
        return authorOperator;
    }
    ...
    public Operator visitGrantUser(GrantUserContext ctx) {
        AuthorOperator authorOperator =
                new AuthorOperator(
                        SQLConstant.TOK_AUTHOR_GRANT, AuthorOperator.AuthorType.GRANT_USER);
        authorOperator.setUserName(ctx.ID().getText());
    ...
    public Operator visitListPrivilegesUser(ListPrivilegesUserContext ctx) {
        AuthorOperator operator =
                new AuthorOperator(
                        SQLConstant.TOK_LIST, AuthorOperator.AuthorType.LIST_USER_PRIVILEGE);
    ...
    public Operator visitListPrivilegesRole(ListPrivilegesRoleContext ctx) {
        AuthorOperator operator =
                new AuthorOperator(
                        SQLConstant.TOK_LIST, AuthorOperator.AuthorType.LIST_ROLE_PRIVILEGE);
    ...
    public Operator visitShowDevices(ShowDevicesContext ctx) {
        if (ctx.prefixPath() != null) {
            return new ShowDevicesOperator(
                    SQLConstant.TOK_DEVICES, parsePrefixPath(ctx.prefixPath()));
        } else {
            return new ShowDevicesOperator(
        ...
        for (ConstantContext constant : ctx.constant()) {
            if (constant.dateExpression() != null) {
                if (!path.equals(TIME_PATH)) {
                    throw new SQLParserException(
                            path.getFullPath(), "Date can only be used to time");
                }
    ...
    /**
     * parse time expression, which is addition and subtraction expression of duration time, now()
     * or DataTimeFormat time.
     *
     * <p>eg. now() + 1d - 2h
     */
            ...
            if (Character.isDigit(ch)) {
                ...
                total +=
                        DatetimeUtils.convertDurationStrToLong(
                                tmp, unit.toLowerCase(), timestampPrecision);
                tmp = 0;
            }
        }
        if (total <= 0) {
