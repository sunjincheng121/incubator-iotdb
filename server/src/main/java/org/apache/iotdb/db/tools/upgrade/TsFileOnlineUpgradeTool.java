...
public class TsFileOnlineUpgradeTool implements AutoCloseable {
    ...
    /**
     * @param movePosition whether move the position of the file reader after reading the magic
     *     header to the end of the magic head string.
     */
    ...
    /** this function does not modify the position of the file reader. */
    public TsDeviceMetadataV1 readTsDeviceMetaData(TsDeviceMetadataIndexV1 index)
            throws IOException {
                ...
                switch (marker) {
                    case MetaMarker.CHUNK_HEADER:
                        ...
                        for (int j = 0; j < header.getNumOfPages(); j++) {
                            ...
                            boolean pageInSamePartition =
                                    checkIfPageInSameTimePartition(pageHeader);
     ...
     * In this case, we have to decode the data to points, and then rewrite the data points to
     * different chunkWriters, finally write chunks to their own upgraded TsFiles
     */
    ...
    private void writePageInSamePartitionToFile(
        ...
        ChunkWriterImpl chunkWriter =
                chunkWriters.getOrDefault(schema, new ChunkWriterImpl(schema));
    ...
    private boolean fileCheck(File oldTsFile) throws IOException {
        ...
        if (!oldTsFile.exists()) {
            logger.error(
                    "the file to be updated does not exist, file path: {}", oldTsFile.getPath());
            return false;
        } else {
        ...
        if (!magic.equals(TSFileConfig.MAGIC_STRING)) {
            logger.error(
                    "the file's MAGIC STRING is incorrect, file path: {}", oldTsFile.getPath());
            return false;
        }
        ...
        if (!versionNumber.equals(TSFileConfig.VERSION_NUMBER_V1)) {
            logger.error(
                    "the file's Version Number is incorrect, file path: {}", oldTsFile.getPath());
            return false;
        }
        ...
        for (Map.Entry<String, List<TimeseriesMetadata>> entry :
            ...
            for (TimeseriesMetadata timeseriesMetaData : entry.getValue()) {
                tsFileResource.updateStartTime(
                        device, timeseriesMetaData.getStatistics().getStartTime());
                tsFileResource.updateEndTime(
                        device, timeseriesMetaData.getStatistics().getEndTime());
            }
        }
