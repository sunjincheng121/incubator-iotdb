...
public class IntTVList extends TVList {
    ...
    public void sort() {
        ...
        if (sortedValues == null || sortedValues.length < size) {
            sortedValues =
                    (int[][]) PrimitiveArrayManager.createDataListsByType(TSDataType.INT32, size);
        }
        sort(0, size);
        clearSortedValue();
        clearSortedTime();
    ...
    public void putInts(long[] time, int[] value, int start, int end) {
        ...
        while (idx < end) {
            ...
            if (internalRemaining >= inputRemaining) {
                // the remaining inputs can fit the last array, copy all remaining inputs into last
                // array
                ...
                // the remaining inputs cannot fit the last array, fill the last array and create a
                // new
                // one and enter the next loop
                System.arraycopy(
                        time, idx, timestamps.get(arrayIdx), elementIdx, internalRemaining);
