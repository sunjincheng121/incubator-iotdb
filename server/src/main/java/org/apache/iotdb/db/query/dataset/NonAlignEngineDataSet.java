            ...
            try {
                synchronized (reader) {
                    ...
                    if ((cachedBatchData[index] != null && cachedBatchData[index].hasCurrent())
                                ...
                                if (offsetArray.get(index) == 0) {
                                    ...
                                    switch (type) {
                                        ...
                                        case DOUBLE:
                                            ...
                                            if (encoder != null && encoder.needEncode(time)) {
                                                doubleValue =
                                                        encoder.encodeDouble(doubleValue, time);
                                            }
                                            ReadWriteIOUtils.write(doubleValue, valueBAOS);
                                            break;
                                        case BOOLEAN:
                                            ReadWriteIOUtils.write(
                                                    batchData.getBoolean(), valueBAOS);
                                            break;
                                        case TEXT:
                                            ReadWriteIOUtils.write(
                                                    batchData.getBinary(), valueBAOS);
                                            break;
                                        default:
                        ...
                        Pair<ByteBuffer, ByteBuffer> timeValueBAOSPair =
                                new Pair(timeBuffer, valueBuffer);
        ...
        for (int seriesIndex = 0; seriesIndex < seriesNum; seriesIndex++) {
            ...
            synchronized (seriesReaderWithoutValueFilterList.get(seriesIndex)) {
                if (blockingQueueArray[seriesIndex].remainingCapacity() > 0) {
                    ManagedSeriesReader reader =
                            seriesReaderWithoutValueFilterList.get(seriesIndex);
