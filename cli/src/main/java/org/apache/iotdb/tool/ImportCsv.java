...
public class ImportCsv extends AbstractCsvTool {
    ...
    private static void loadDataFromCSV(File file) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file));
            ...
            if (cols.length <= 1) {
                System.out.println(
                        "The CSV file " + file.getName() + " illegal, please check first line");
                return;
            }
            ...
            for (int i = 1; i < cols.length; i++) {
                splitColToDeviceAndMeasurement(
                        cols[i], devicesToPositions, devicesToMeasurements, i);
            }
    ...
    public static void importCsvFromFile(
        ...
        try {
            ...
            System.out.println(
                    "Encounter an error when connecting to server, because " + e.getMessage());
            ...
            if (session != null) {
                try {
                    ...
                    System.out.println(
                            "Encounter an error when closing the connection, because "
                                    + e.getMessage());
                }
            }
        }
    ...
    private static int getFileLineCount(File file) throws IOException {
        ...
        try (LineNumberReader count = new LineNumberReader(new FileReader(file))) {
            while (count.skip(Long.MAX_VALUE) > 0) {
                // Loop just in case the file is > Long.MAX_VALUE or skip() decides to not read the
                // entire file
            }
    ...
    private static void splitColToDeviceAndMeasurement(
        ...
        if (col.length() > 0) {
            if (col.charAt(col.length() - 1) == TsFileConstant.DOUBLE_QUOTE) {
                ...
                if (endIndex < 0) {
                    putDeviceAndMeasurement(
                            "", col, devicesToPositions, devicesToMeasurements, position);
                } else {
                    putDeviceAndMeasurement(
