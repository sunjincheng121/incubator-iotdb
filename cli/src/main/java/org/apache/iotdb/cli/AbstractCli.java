...
public abstract class AbstractCli {
    ...
    static Options createOptions() {
        ...
        Option timeFormat =
                new Option(ISO8601_ARGS, false, "Display timestamp in number(optional)");
    ...
    static String checkRequiredArg(
        ...
        if (str == null) {
            if (isRequired) {
                ...
                String msg =
                        String.format(
                                "%s: Required values for option '%s' is null.",
                                IOTDB_CLI_PREFIX, name);
                throw new ArgsErrorException(msg);
            } else {
                return defaultValue;
    ...
    static String[] processExecuteArgs(String[] args) {
        ...
        if (index >= 0
            ...
            // some bashes may not remove quotes of parameters automatically, remove them in that
            // case
    ...
    private static void setTimeZone(String specialCmd, String cmd, IoTDBConnection connection) {
        ...
        if (values.length != 2) {
            println(
                    String.format(
                            "Time zone format error, please input like %s=+08:00", SET_TIME_ZONE));
            return;
        }
        try {
    ...
    private static void setFetchSize(String specialCmd, String cmd) {
        ...
        if (values.length != 2) {
            println(
                    String.format(
                            "Fetch size format error, please input like %s=10000", SET_FETCH_SIZE));
            return;
        }
        try {
    ...
    private static List<List<String>> cacheResult(
        ...
        if (resultSet instanceof IoTDBJDBCResultSet) {
            ...
            for (int i = 1; i <= columnCount; i += 2) {
                ...
                timeList.add(
                        resultSetMetaData.getColumnLabel(i).substring(0, TIMESTAMP_STR.length()));
                lists.add(timeList);
